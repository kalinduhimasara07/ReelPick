...
def home():
    ...
    - If no history: Shows 'initial' state (just search).
    - If history: Gathers all unique recommendations and searched movies
      from history and shows 'browse' state.
    """
    ...
    if session['search_history']:
        # User has history. Gather all unique recommendations.
        
        # Use dictionaries to store unique movies by ID
        all_recommendations = {}
        all_searched_movies = {}

        # Iterate through history in reverse to prioritize newer searches
        for result_set in reversed(session['search_history']):
            
            # Add all recommendations from this search
            for movie in result_set['recommendations']:
                if movie['movie_id'] not in all_recommendations:
                    all_recommendations[movie['movie_id']] = movie
            
            # Add the movie that was searched for
            searched_movie = result_set['searched_movie']
            if searched_movie['movie_id'] not in all_searched_movies:
                 all_searched_movies[searched_movie['movie_id']] = searched_movie

        return render_template('index.html',
                               app_name="ReelPick",
                               # Pass the unique recommendations as a list
                               recommendations=list(all_recommendations.values()),
                               # Pass the unique searched movies as a list
                               searched_movies=list(all_searched_movies.values()),
                               state='browse')

    # No search history, show the initial empty state.
...
def autocomplete():
    ...
    suggestions = df['title'][df['title'].str.contains(
        search_term, case=False, na=False)].tolist()
...
def recommend():
    ...
    error_message = None
    state = 'initial' # Default state if error
